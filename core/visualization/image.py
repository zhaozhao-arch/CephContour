...
class ImshowInfosContextManager(BaseFigureContextManager):
    ...
    def put_img_infos(self,
        ...
        if out_file is not None:
            save_success = cv2.imwrite(out_file, img_save)

            if save_success:
                print(f"图片保存成功：{out_file}")
            else:
                # 保存失败时输出错误信息，帮助定位问题
                print(f"警告：图片保存失败！路径：{out_file}")
                # 可选：进一步排查失败原因（如路径不存在、权限问题等）
                import os
                dir_name = os.path.dirname(out_file)
                if not os.path.exists(dir_name):
                    print(f"原因：保存目录不存在 - {dir_name}")
                    # 可选：自动创建目录并重试
                    os.makedirs(dir_name, exist_ok=True)
                    retry_success = cv2.imwrite(out_file, img_save)
                    if retry_success:
                        print(f"自动创建目录后保存成功：{out_file}")
                    else:
                        print(f"创建目录后仍保存失败：{out_file}")
                elif not os.access(dir_name, os.W_OK):
                    print(f"原因：无写入权限 - {dir_name}")
